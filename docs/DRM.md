...
> Unfortunately, while the Core/Activation.dll hook works, the CreateEAWebkitInstance hook does not. SimCity crashes with an error saying the DLL could not be loaded. Perhaps this has to do with not exporting the cairo functions or that one C++ operator.
